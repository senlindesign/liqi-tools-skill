...
<img width="1319" height="963" alt="Screenshot 2026-08-10 at 10 29 37" src="https://github.com/user-attachments/assets/fd44f712-30dc-4565-989b-bb342d86f149" />
...
使用 `interviews/` 及相关衍生数据时，请：

- 保留「利器」及相关创作者的署名；
- 提供原始访谈链接和适用的许可协议链接；
- 标明对原始内容所作的修改；
- 遵守 CC-BY-NC-SA 条款，不将这些内容用于商业目的。

本项目的许可不涵盖访谈中可能涉及的第三方图片、商标、肖像权、隐私权、人格权或其他权利。使用者应在需要时自行取得相应授权。

本项目不会使用「利器」访谈语料训练或微调模型。Skill 仅在用户主动提出相关请求时，检索并读取相关访谈内容，将其作为生成回答时的临时参考信息。对话数据的收集、保存和处理由实际运行平台负责，具体规则以该平台的服务条款和隐私设置为准。
